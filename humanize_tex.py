...
humanized_body = r"""% ============================================================
\begin{abstract}
Outcome-Based Education (OBE) is a very important part of engineering colleges today, especially for getting NBA and NAAC accreditation. However, making OBE work in real life takes up a lot of time for teachers. Faculty members usually have to map Course Outcomes (COs) to Program Outcomes (POs) by hand and calculate attainment scores using complicated Excel sheets. Doing this manually often leads to calculation mistakes and inconsistent results. To solve these problems, we built a web-based OBE Management System using the Gemini API and LangChain.

Instead of relying on spreadsheets, our system uses AI to automatically create CO-PO matrices and check if exam questions match Bloom's Taxonomy. To make sure the AI doesn't make things up, we forced it to output structured JSON data using Pydantic schemas. We also fixed a common math mistake where colleges simply average internal exams that have different maximum marks. Our system ensures the final calculations follow NBA rules exactly. During early testing at the MIT Academy of Engineering, our tool cut the time needed for CO-PO mapping by over 97\% and got great feedback from users. Overall, this system offers a practical and accurate way to modernize how colleges handle accreditation.
\end{abstract}
